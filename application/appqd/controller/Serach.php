...
class Serach extends Controller
{
	public function index(){ //新闻列表
		if(request()->post()){  
			$val = input('val');
			$arr=Db("article")->where('autor|title|descr','like','%'.$val.'%')->paginate(10);    
			$arrsa=Db("notic")->where('title|descr','like','%'.$val.'%')->paginate(10);

			if(!empty($arr) || !empty($arrsa) ){ 
				return json_encode(array('state'=>200,'msg'=>'查询成功','xx'=>$arr,'oo'=>$arrsa));  
			}else{
